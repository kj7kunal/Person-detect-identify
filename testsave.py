...
def mark_classes_saveinstances(x,orig_img,id_,i):
	label = class_names[int(x[-1])]
	if label=="person":
		if id_%3==0:
			cropped = orig_img[x[2].int():x[4].int(),x[1].int():x[3].int()]
			cv2.imwrite("./saved/"+str(id_)+str(i)+".jpg",cropped)
...
if __name__=="__main__":
	...
	if args["video"]:
		...
		while cap.isOpened():
			...
			list(map(lambda x,i: mark_classes_saveinstances(x,orig_img,currFrame,i),output,range(len(output))))
