...
class Img2Vec():
    ...
    def __init__(self, model='resnet34', layer_output_size=512):
        ...
        self.np2PIL = transforms.ToPILImage()
    ...
    def get_vec(self, img):
        ...
        # cv2
        if self.CUDA:
            image = Variable(self.normalize(self.to_tensor(self.scaler(self.np2PIL(img)))).unsqueeze(0)).cuda()
        else:
            image = Variable(self.normalize(self.to_tensor(self.scaler(self.np2PIL(img)))).unsqueeze(0))
